...
void read_vect(FILE *fp, int size, int vect[])
 {
  for(int i=0;i<size;i++)
   {
    fscanf(fp,"%d ", &vect[i]);
...
//first case size of the hash larger than size of the message (size1>size 2)
void encrypt1( int size1, int size2, int hash[], int message[])
{
 FILE *fp;
 fp=fopen("cipher.txt","w+");
 for(int i=0;i<size2;i++)
  {
   //cipher[i]=(hash[i]+message[i])%2;
   fprintf(fp,"%d ",(hash[i]+message[i])%2);
  }
...
//Second case is where the size of the message is larger than size of the hash(size2>size 1)
void encrypt2(int size1, int size2, int hash[], int message[])
 {
  int nb_blocks=ceil((float)size2/(float)size1);
  FILE *fp;
 fp=fopen("cipher.txt","w+");
  for(int i=0;i<nb_blocks;i++)
   {
    for(int j=0;j<size1;j++)
     {
      //cipher[j+(i*size1)]=(message[j+(i*size1)]+hash[j])%2;
      fprintf(fp,"%d ",(message[j+(i*size1)]+hash[j])%2);
     }
   }
   fclose(fp);
 }
void encrypt( FILE *fp1, FILE *fp2)
 {
  ...
  int size1=file_size(fp1)-1, size2=file_size(fp2)-1;
  //reading the content of the files
  int message[size2],key[size1];
  read_vect(fp1,size1,key);
  read_vect(fp2, size2,message);
  //comparison to create correct ciphertext size
  if(size1>size2)
   {
    encrypt1(size1,size2,key,message);
   }
  else
   {
   encrypt2(size1,size2,key,message);
   }
  }
  //first case size of the hash larger than size of the message (size1>size 2)
void decrypt1( int size1, int size2, int hash[], int cipher[])
{
 FILE *fp;
 ...
 for(int i=0;i<size2;i++)
  {
   //cipher[i]=(hash[i]+message[i])%2;
   fprintf(fp,"%d ",(hash[i]+cipher[i])%2);
  }
 for(int i=size2;i<size1;i++)
  {
  //cipher[i]=hash[i];
...
//Second case is where the size of the message is larger than size of the hash(size2>size 1)
void decrypt2(int size1, int size2, int hash[], int cipher[])
 {
  int nb_blocks=ceil((float)size2/(float)size1);
  FILE *fp;
 fp=fopen("message.txt","w+");
  for(int i=0;i<nb_blocks;i++)
   {
    for(int j=0;j<size1;j++)
     {
      //cipher[j+(i*size1)]=(message[j+(i*size1)]+hash[j])%2;
      fprintf(fp,"%d ",(cipher[j+(i*size1)]+hash[j])%2);
     }
   }
   fclose(fp);
 }
void decrypt( FILE *fp1, FILE *fp2)
 {
  ...
  int size1=file_size(fp1)-1, size2=file_size(fp2)-1;
  //reading the content of the files
  int cipher[size2],key[size1];
  read_vect(fp1,size1,key);
  read_vect(fp2, size2,cipher);
  //comparison to create correct ciphertext size
  if(size1==size2)
   {
    decrypt1(size1,size2,key,cipher);
   }
  else
   {
   decrypt2(size1,size2,key,cipher);
   }
  }
...
void hashing ( char * name)// printing the hash value into a file
 {
  ...
  p=popen(str,"r");
  char temp;
  short unsigned int vect[128];
  
  for(int i=0;i<32;i++)
  {
   temp=fgetc(p);
   int2bin(temp,128,vect);
   for(int j=0;j<128;j++)
    {
   fprintf(fp,"%d ",vect[j]);
   }
